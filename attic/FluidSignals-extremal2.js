...
const $fluidSignalsScope = function (fluid) {
     ...
     * @param {Boolean} [availChange] - This update sweep has resulted from a cell's availability change
     */
    fluid.cell.markStale = function (cell, state, markedSources, dirtyFrom, availChange) {
        console.log("markStale for " + cell.name, " state ", state, " availChange ", availChange);
        fluid.cell.queueEffect(cell);

        // If we were previously clean, then we know that we may need to update to get the new value
        // If there is an availChange we need to push up the graph to eagerly rediscover effects and mark as
    ...
    fluid.cell.triageChange = function (oldValue, newValue) {
        const oldUnavail = fluid.isUnavailable(oldValue);
        const newUnavail = fluid.isUnavailable(newValue);
        let availChange = false;
        let newConcrete = newValue;
        if (oldUnavail !== newUnavail) {
            // Special carveout: We've come from the branch in fluid.cell.update which has short-circuited arguments and lifted old value to pending
            // We will need to traverse the graph and look for effects
            if (newUnavail && newUnavail.variety === "pending") {
                newConcrete = newValue.staleValue;
                availChange = false;
            } else {
                availChange = true;  // availability class transition
            }
        } else if (oldUnavail) {
            // Both unavailable: variety change is observable; otherwise treat as same
            availChange = oldValue.variety !== newValue.variety;
        }
        return {
            availChange,
            notifiableChange: availChange ? availChange : !fluid.cell.equals(oldValue, newConcrete)
        };
    };
    ...
    fluid.cell.propagateValueChange = function (cell, availChange, notifiableChange) {
        console.log("propagateValueChange with availChange ", availChange, " and notifiableChange ", notifiableChange);
        // Misleading original comment:
        // handles diamond dependencies if we're the parent of a diamond.
        if (notifiableChange && cell._observers) {
            const consumedSources = cell._consumedSources;
    ...
    fluid.cell.commitValue = function (cell, newValue) {
        ...
        const { availChange, notifiableChange } = fluid.cell.triageChange(oldValue, newValue);

        fluid.cell.propagateValueChange(cell, availChange, notifiableChange);
    ...
    fluid.cell.updateComplete = function (newValue, cell) {
        ...
        if (!fluid.isUnavailable(newValue)) {
            console.log("Update complete for " + (cell._isEffect ? " effect " : "") + cell.name + ", marking clean with new value ", newValue);
            cell._state = CacheClean;
        }
    ...
    fluid.cell.prototype.set = function (value, options) {
        ...
        if (!fluid.isConfigUnavailable(value)) {
            console.log("SET acted Update complete for " + (this._isEffect ? " effect " : "") + this.name + ", marking clean");
