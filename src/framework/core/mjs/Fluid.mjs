...
import "./FluidModules.mjs";
     ...
     */
    fluid.copy = function (tocopy, segs = []) {
        fluid.testStrategyRecursion("fluid.copy", segs);
        const copyMember = function (value, key) {
            segs.push(key);
            const togo = fluid.copy(value, segs);
            segs.pop();
            return togo;
        };
    ...
    /**
     * Set a value at a given path within an object or array, without mutating the original structure.
     * Creates and returns a new structure with the updated value.
     *
     * @param {any} root - The root object to copy and modify.
     * @param {String|Array<String|Number>} path - A path expressed as a dot-separated String or an Array of segments.
     * @param {any} newValue - The new value to set at the specified path.
     * @return {any} A new object with the updated value.
     */
    fluid.setImmutable = function (root, path, newValue) {
        const segs = fluid.pathToSegs(path);
        if (segs.length === 0) {
            return newValue;
        } else {
            let newRoot = fluid.shallowCopy(root);
            let current = newRoot;
            ...
            for (let i = 0; i < segs.length - 1; ++i) {
                ...
                const copied = fluid.shallowCopy(next);
                current[seg] = copied;
                current = copied;
            }

            current[segs[segs.length - 1]] = newValue;
            return newRoot;
        }
    };
    ...
    fluid.mergeLayers = function (target, segs, layerMap, layers, mergeRecords) {
        ...
        allKeys.forEach(key => {
            ...
            fluid.set(layerMap, [...segs, key, $m, "source"], mergeRecords[lastIndex].mergeRecordName);
        });
    };
     ...
     * @param {String[]} [segs=[]] - The current path segments during recursive traversal.
     */
    fluid.extractPolicy = function (layer, layerMap, segs = []) {
        const recurse = function (value, key) {
            segs.push(key);
            fluid.extractPolicy(value, layerMap, segs);
            segs.pop();
        };
        if (fluid.isPlainObject(layer, true)) {
    ...
    fluid.mergeLayerRecords = function (root, mergeRecords) {
        ...
        layers.forEach(layer => fluid.extractPolicy(layer, layerMap));
